...
npm run probe:mcp             # drive the real app and call all thirteen MCP tools over a real
...
npm run probe:identity        # which observed payloads are ONE value. The Knowledge layer's
                              ...
                              # Measured 2026-09-06 over 13 recordings, BY FACT ID because
                              # two facts read focus_change: display_topology 13 occurrences
                              # / 9 raw / 3 folded, focused_app 102 / 71 / 7, focused_window
                              # 102 / 71 / 29 with 19 unidentified, visited_page 50 / 23 / 21
                              # with 3, keyboard_layout 13 / 1 / 1. THE NUMBERS MOVE -- every
                              # one of these changed when the library gained ONE recording,
                              # which is why they are re-rendered and not quoted.
                              # IT ALSO SWEEPS THE HALF-LIFE the value ordering uses, and the
                              # columns SPLIT TIEBREAKS FROM REAL OVERRIDES for probe:baseline's
                              # reason -- its first version printed "3 of 5 facts reordered" at
                              # every half-life including 90d on a 20-day span, which was
                              # recency separating pairs the lifetime rule could only separate
                              # by key. IT SWEEPS THE STREAMS THE APP RENDERS, not the
                              # unexcluded ones the fold table uses -- the first version swept
                              # the wrong split and reported 0 leading values moved where 3 do.
                              # Measured: 54 real overrides at the shipped 14d (106 at 7d, 35
                              # at 30d and 90d), 172 tiebreaks, and 3 of 5 facts whose LEADING
                              # value differs from what the lifetime tally would have shown.
                              # IT CALLS THE APP'''S OWN GATHERER
                              # (`app/src/main/knowledge-view.ts`) rather than keeping a copy
                              # of the reading -- a probe re-implementing what it measures is
                              # the ax-dump/ax-exec drift hazard. It also prints WHAT THE
                              # RECORDER EXCLUSION COSTS each fact and the counterfactual that
                              # justifies scoping it: 263 of 5851 events dropped, focused_app
                              # 7 -> 6, focused_window 29 -> 28 and visited_page 21 -> 19,
                              # against 6 of 13 recordings that would lose their display AND
                              # their keymap outright were the exclusion applied to the
                              # AMBIENT facts too.
...
- **IDENTITY IS A DECLARED PROJECTION, AND THE FIELD THE OS RE-MINTS IS THE DECOY.** `currentValue` deliberately does not decide whether two values are the same value; `foldByIdentity` (`src/knowledge/identity.ts`) does, and a `FoldedFact` **is** a `KnowledgeFact`, so the resolver reads one with `facts.ts` unmodified — if that file needs an edit, the seam is wrong. Each fact type declares the form that decides sameness (`src/knowledge/identities.ts`), and a projection **enumerates what it KEEPS**, so a new field on `DisplayInfo` does not silently become a discriminator. Measured 2026-09-06 over 13 recordings: `display_topology` 13 occurrences / 9 raw / **3** — `DisplayInfo.id` is stable for the BOOT and no longer, and the same primary carried seven ids; `focused_app` 102 / 71 / **7** bundle ids, where the target number is QUESTION-DEPENDENT so a consumer wanting the window count wants a second identity, not an edit — and **cycle 3 declared it**: `focused_window` reads the same 102 events at `(bundleId, title)` and returns **29**, plus **19 unidentified**, because nearly a fifth of real focus events carry no title. **A FACT IS THEREFORE ADDRESSED BY `IdentityDeclaration.id`, NOT BY ITS EVENT KIND** — two declarations read `focus_change`, so a card keyed on the kind collides and `get_fact` cannot say which one is meant. `visited_page` 50 / 23 / **21** plus **3** unidentified. **`VISITED_PAGE` CALLS `urlPrefix` and writes no rule of its own** — a hand-written normalizer merged the same single pair, so the tie went to the rule that ships, and a private URL rule in `src/knowledge/` is the exact regression `src/index.ts` exports `urlPrefix` to prevent. Sameness as a MEASURE (embed and cluster) was declined for want of ground truth on a 12-recording library — `probe:embed`'s first version is why. **A DECLARED IDENTITY IS ONLY HALF A VALUE; THE OTHER HALF IS THE LABEL, AND THE LABEL MUST BE INJECTIVE OVER THE CANONICAL FORM.** `displayLabel` rendered four of the six fields the identity keeps, and the real library holds the pair that breaks: two docked configurations differing ONLY in the external panel's `y` (−797 and −706) printed one identical string — two values, one label, which is a duplicate React key, two rows nobody can tell apart, two identical blocks in `get_fact`, and a `current` field (a LABEL) that names neither. `test/knowledge-view.test.ts` asserts injectivity per fact, so a sixth declaration is covered the day it is added. `npm run probe:identity` re-renders all of it.
- **AN IDENTITY IS DECLARED BY A CONSUMER, AND `attribution` IS THE SECOND THING A DECLARATION CARRIES.** `keymap_change` was recorded as having NO identity on a measurement that is still right — 13 occurrences, one payload, a projection buys zero VALUES. Cycle 2 declared one anyway, because a projection also decides what a consumer SEES and the raw payload is `{ layoutId, entries: { …70 keycode mappings… } }`. `KEYBOARD_LAYOUT` is also the one `exclusive` declaration, which makes it **the one fact that returns a value**: the other four are `coexisting`, so a screen built on them alone shows DeskRAG declining four times and teaches that refusing is all the layer does. **THE TWO AXES ARE NOT INDEPENDENT AND THE FILE HEADER SAYS SO**: on all five declarations `focused-app` implies `coexisting`, because those facts are ACTIVITY distilled — which applications, which windows, which sites — and activity cannot supersede. The two `ambient` ones are the ENVIRONMENT, which is the paper's Knowledge. A sixth declaration should ask which of the two it is before taking either field by analogy. **THE RECORDER EXCLUSION IS SCOPED BY FACT TYPE**: `IdentityDeclaration.attribution` is `focused-app` or `ambient`, data beside exclusivity on `text-profiles.ts`'s precedent. Measured — applying `excludeFocusedApps` to the ambient facts costs **6 of 13 recordings their `display_change` and 6 of 13 their `keymap_change` outright**, because both are sampled at session start while the recorder is still frontmost, and the docked display configurations survived that by luck: each is a SINGLE observation. A display topology is not work, and `excludeFocusedApps` exists to drop work.
- **A KNOWLEDGE VALUE LIST IS ORDERED BY RECENCY-WEIGHTED EVIDENCE, AND THAT IS §4'S DEFECT REPAIRED IN THE LAYER BUILT FROM §4.** The first version ranked values by a RAW LIFETIME TALLY — distinct recordings, then observations — which is `edgeCost`'s defect verbatim: a display setup used ten times last spring and abandoned outranks one used three times last week, forever. It had a visible consequence too, because `currentValue` picks the most recently observed value while the list ranked by lifetime count: a layout seen in three recordings sat ABOVE the one that had replaced it, directly under a verdict naming the replacement. The weight is `0.5 ** (Δ / halfLife)` per distinct recording — the SAME expression as `edgeCost`'s `evidenceOf` and `walk-analysis`'s `wayWeight`, reusing `DEFAULT_HALF_LIFE_MS` so the app has one half-life and not three — and **an undatable recording keeps its whole vote**, because recency may discount evidence we can date and must never penalise evidence we merely cannot. **`stabilityOf` IS NOT TOUCHED**: a tier is the paper's evidence threshold, a `COUNT(DISTINCT session_id)`, and a weighted count would be a fraction wearing the word "recordings". **The weight never leaves `knowledge-view.ts`** — it is a ratio, so what ships is the rank it produced plus `lastObservedAt`, a MOMENT, which is what makes a `core · 8 recordings` row sitting below a `prediction · 1 recording` one legible instead of broken. Measured 2026-09-06 over 13 recordings: **54 real overrides at the shipped 14-day half-life** (106 at 7d, 35 at 30d and 90d) against 172 tiebreaks, and — the number that justifies it — **3 of 5 facts lead with a different value than the lifetime tally would have shown**. Applications is the readable case: Chrome, seen in 5 recordings but seen TODAY, now leads TextEdit and Calculator at 6 recordings each and a fortnight cold. Unlike `chooseBaseline`'s sweep this ships **ON**, and the constant is not inert either — the leading value of one fact moves at 7d/30d/90d, so `DEFAULT_HALF_LIFE_MS` stays as unswept a disclosure here as it is there.
- **`KnowledgeValueDTO` CARRIES A `key` AND AN `isCurrent`, AND BOTH EXIST BECAUSE A LABEL IS A RENDERING.** The screen keyed its rows on the label and named the current value only in the verdict paragraph above them; with a non-injective label that is one React key for two rows, and with a recency order the current value is usually but not always first. The key is `stableKey` of the canonical form — the same function `foldByIdentity` groups with, never a second serializer — and `isCurrent` is set from the comparison `currentValue` already makes. Nothing downstream matches a value by its text.
...
- **The MCP endpoint is READ-ONLY BY CONSTRUCTION**, guarded by `test/mcp.readonly.test.ts`. **The Host check is what closes DNS rebinding** — the Origin check cannot. There is deliberately no token, and it shows no score. **Thirteen tools, and the guard's `^(search|get|list)_` rule NAMED one of them**: `find_habit` could not ship, because widening that prefix to admit `find_` would trade a structural property for a word. The guard also reads the `ExperienceReader` body with NO word boundaries, so an `inputs` parameter fails on `put` and an inline `startedAt` fails on `start` — the reader takes `texts` and declares its return types outside the interface. **`search_habits` discloses the corpus BEFORE the ranking** (`RANKING_MIN_HABITS = 5`, unswept), and **`steps.json` carries no recorded slot value at all**, `showSamples` unconsulted — `habit-marks.ts`'s rule, that a payload has no toggle so it carries no values.
- **KNOWLEDGE IS A PURE PROJECTION IN `knowledge-view.ts` AND STAYS ONE, because the alternative is testable only by launching the app.** `DeskRagService.knowledge()` does the I/O and nothing else, mirroring `flows()`; the fold, the fact-scoped exclusion, all three refusals and the label rendering are in the projection, which is in the root suite. **It is in `app/src/main/` and not `src/knowledge/`** — the inputs are `TraceEvent`s, a settings-held exclusion list and session start times, and WHICH APPLICATIONS COUNT AS THE RECORDER IS THE APP'S POLICY, which is why `excludeFocusedApps` takes its predicate injected. **A LABEL IS RENDERED ONCE**, in main, so the screen and `list_facts` are byte-identical on the one string they are both about — two renderers is how a habit's clipboard string and `get_habit`'s would come to disagree. **NOTHING IS STORED AND THAT IS A MEASUREMENT**: the whole pipeline is 4.63ms on the real library (5079 events; 3.77ms of it reading rows) and 34.13ms for a hundredfold fold, so `docs/research/persistence-layers.md` §6.2 closes with no table and no bucket.
...
- **THE KNOWLEDGE SCREEN PUTS THE ANSWER AND THE REFUSAL IN ONE SLOT, and on the real library four of its five cards refuse.** A `coexisting` fact has no current value — a laptop docked some days has two display setups and both are true — so a refusal is an ANSWER and drawing it anywhere else would make declining look like an error state. Its evidence column is `stabilityOf`'s tier plus the date the value was last seen: **a word, a count of recordings and a date**, printable exactly where `FrameResult.score` is not, and there is no percentage on the screen. Rows are ordered by recency-weighted evidence and the current one carries a rule on its leading edge, so the list cannot contradict the verdict above it. **NOTHING TRUNCATES, BUT A LONG LIST FOLDS**: past `MAX_FACT_VALUES` (12) the remainder is counted in a caveat on `IndexShare`'s rule — `focused_window` alone lands at 29 — and `get_fact` is deliberately NOT capped, because checking a fold against a truncated list is not checking it. The chip says `ambient` or `recorder excluded` because after the attribution measurement that is a claim the card has to be able to make. **It sits AFTER Habits** — `App.tsx` documents the Flows→Habits adjacency as a stated relationship ("a habit is made FROM a flow"), and a new screen does not sever one.
