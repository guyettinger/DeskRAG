...
- **THE RECORDER IS NOT PART OF THE WORK IT RECORDS, and it is filtered at LIFT, not in the projection.** A session is started and stopped from the app, so every recording was bracketed by it: measured on the real store, all 4 routes opened `Electron — localhost →` and the one kept skill's step 1 was `n0 — no state → Electron — localhost` with **no actions on the edge**. `excludeFocusedApps` drops every event that occurred while an excluded app was frontmost — one rule covering the leading stretch, the trailing stretch and a mid-recording visit. At lift because `rebuildGraph` re-lifts from events and AX walks already on disk, so it **repairs recordings already taken**; a filter in `graph-view.ts` would relabel the routes and leave the cards and the skill steps behind. 28 nodes/61 edges → 22/49, 4 routes → the same 4 starting at real work.
...
- **`bindSkill`'s LIVE key is the one that claims, and two callers read the stored key by mistake.** A rebuild re-keys every route, so a skill's stored `routeKey` goes stale *by design* and is disclosed rather than repaired. `unclaimedRoutes` was fed the stored key and offered a kept skill's own route straight back as a new proposal; `probe:transfer` matched the raw key and called every kept skill orphaned. Both now resolve through `bindSkill` — grep for `binding.routeKey` before adding a third.
