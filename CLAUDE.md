...
npm run probe:identity        # which observed payloads are ONE value. The Knowledge layer's
                              ...
                              # url_change 44 / 19 / 17 with 3 unidentified, keymap_change
                              # 12 / 1 / 1. IT CALLS THE APP'''S OWN GATHERER
                              ...
                              # justifies scoping it: 253 of 5079 events dropped, focus_change
                              # 7 -> 6 and url_change 17 -> 15, against 6 of 12 recordings
                              # that would lose their display AND their keymap outright were
                              # the exclusion applied to the AMBIENT facts too.
...
- **THE KNOWLEDGE SCREEN PUTS THE ANSWER AND THE REFUSAL IN ONE SLOT, and on the real library three of its four cards refuse.** A `coexisting` fact has no current value — a laptop docked some days has two display setups and both are true — so a refusal is an ANSWER and drawing it anywhere else would make declining look like an error state. Its evidence column is `stabilityOf`'s tier: **a word and a count of recordings**, printable exactly where `FrameResult.score` is not, and there is no percentage on the screen. The chip says `ambient` or `recorder excluded` because after the attribution measurement that is a claim the card has to be able to make. **It sits AFTER Habits** — `App.tsx` documents the Flows→Habits adjacency as a stated relationship ("a habit is made FROM a flow"), and a new screen does not sever one.
