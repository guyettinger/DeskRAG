...
- **The Indexing ladder's ROW is execution order and its COLUMN is dependency depth**, because `runStages` is a strictly sequential loop and a free DAG layout would assert a concurrency the app does not have. `trace` is depth 2 and runs LAST, so deriving the row from `needs` misplaces it silently. **Wires get one CHANNEL each** (`index-graph-view.ts`) — bowed curves put 16 of 21 wires in the same few pixels and drew static, which every DOM assertion passed.
