...
function splitImageString(rows, cols) {
  let s = "";
...
const IMAGE_ID = enc("<image>").filter((id) => id >= 50368)[0];
console.log(`length ${full.length}, image tokens ${full.filter((i) => i === IMAGE_ID).length}`);
