...
import { findPath } from "../../src/replay/plan.js";
import { isRepairStep, isSupersededStep } from "../../src/replay/types.js";
import { observe, windowOriginOf } from "../../src/replay/observe.js";
import { executeRun } from "../../src/replay/run.js";
import type { Actuator } from "../../src/replay/types.js";
import type {
  Action,
  Anchor,
  Graph,
...
const DATA = process.env.DESKRAG_DATA ?? DATA_DIR;
const GRAPH_ID = process.env.DESKRAG_GRAPH ?? "default";

// NAMES WITHOUT DASHES — `scripts/lib/args.ts` owns the `--` prefix. This
// file's own parser used to want them written in, and passing `"--offline"`
// through the shared one looks for `----offline`: it does not throw, it
// silently falls through to the DEFAULT mode. Caught by running the probe.
const has = (f: string): boolean => flag(f);
const val = (f: string): string | undefined => arg(f);

/** Every mutating Actuator method, blocked. */
const MUTATORS = ["activate", "moveTo", "click", "dragPath", "scroll", "key"];

function readOnly<T extends Actuator>(actuator: T): T {
  return new Proxy(actuator, {
    get(target, prop, recv) {
      if (MUTATORS.includes(String(prop))) {
        return () => {
          throw new Error(`replay-probe is read-only: refused ${String(prop)}()`);
        };
      }
...
const anchorOf = (a: Action): Anchor | undefined =>
  (a as { anchor?: Anchor; from?: Anchor }).anchor ?? (a as { from?: Anchor }).from;

/**
 * The anchor of an action `isSpatial` already accepted.
 *
 * Every spatial kind carries one by construction — `click`/`hover`/`scroll` an
 * `anchor`, `drag` a `from` — so a missing one is a malformed graph, not a case
 * to skip silently.
 */
const spatialAnchor = (a: Action): Anchor => {
  const anc = anchorOf(a);
  if (anc === undefined) throw new Error(`${a.kind} action has no anchor — malformed graph`);
  return anc;
};

function descriptorsOf(anchor: Anchor | undefined): string[] {
  const d: string[] = [];
  const ax = anchor?.ax;
  if (ax) {
...
const hasAxLayer = (anchor: Anchor | undefined): boolean => anchor?.ax !== undefined;
const isSpatial = (a: Action): boolean =>
  a.kind === "click" || a.kind === "hover" || a.kind === "scroll" || a.kind === "drag";

const appOf = (node: TraceNode | undefined): string | undefined =>
  node?.predicates.find((p) => p.kind === "app")?.args.app as string | undefined;

/** Deductive upper bound on an edge's AX rate: anchors with no ax layer can never reach a rung. */
function axUpperBound(actions: readonly Action[]): number {
 ...
 */
function afterSupersession(
  edge: TraceEdge,
  crosses: boolean,
  toApp: string | undefined,
): Action[] {
...
interface LocateResult {
  ...
  candidates: number;
  ambiguous: boolean;
  all: { id: string; n: number; app: string | undefined }[];
}

function locateNode(observed: readonly Predicate[], nodes: readonly TraceNode[]): LocateResult {
  ...
  const top = sorted[0];
  if (top === undefined) return { candidates: 0, ambiguous: false, all: [] };
  const tied = sorted.filter((n) => n.predicates.length === top.predicates.length);
  return {
    ...(tied.length === 1 ? { nodeId: top.id } : {}),
...
function offline(graph: Graph): void {
 ...
 */
async function waitForApp(actuator: Actuator): Promise<boolean> {
...
async function live(graph: Graph, goalId: string | undefined): Promise<void> {
  ...
  try {
    ...
    if (loc.all.length > 1) {
      const [a, b] = loc.all;
      const nodes = new Map(graph.nodes.map((n) => [n.id, n]));
      const pa = a === undefined ? undefined : nodes.get(a.id);
      const pb = b === undefined ? undefined : nodes.get(b.id);
      const nests =
        pa !== undefined &&
        pb !== undefined &&
        pb.predicates.every((p) =>
          pa.predicates.some((q) => JSON.stringify(q) === JSON.stringify(p)),
        );
 ...
 */
async function sweep(graph: Graph): Promise<void> {
  ...
  try {
    ...
    for (const e of graph.edges) {
      ...
      for (const a of e.actions) {
        ...
        console.log(
          `  ${(e.id.split(":").pop() ?? e.id).padEnd(4)} ${(ownerApp ?? "?").padEnd(15)}` +
 ...
 */
type Keymap = Parameters<typeof executeRun>[0]["keymap"];

function sessionKeymap(store: DualStore): Keymap | undefined {
  for (const s of store.listSessions()) {
    for (const e of store.getEventsBySession(s.id)) {
      const data = e.data as { entries?: unknown } | undefined;
      if (e.kind === "keymap_change" && data?.entries !== undefined) return data as Keymap;
    }
  }
  return undefined;
}
    ...
    const out = await executeRun({
      ...
      arm: async (plan) => {
        ...
        for (const s of plan.steps) {
          // The exported guards, not a hand-rolled `"repair" in s` — they key on
          // the field unique to each variant and keep `PlannedAction` as the
          // fall-through, which is the contract `types.ts` documents.
          if (isRepairStep(s)) {
            console.log(`  activate  ${s.app}${s.launch ? " (launching)" : ""}`);
          } else if (isSupersededStep(s)) {
...
async function liveRun(
  ...
  try {
    ...
    const out = await executeRun({
      graph,
      actuator: sidecar,
      // The same fallback `planOnly` uses: no keymap would make `type` look
      // unarmable for a reason that has nothing to do with resolution.
      keymap: keymap ?? { layoutId: "none", entries: {} },
      goalNodeId: goal.id,
      arm: async (plan) => {
        segment++;
 ...
 */
async function diagnoseFrontmost(seconds: number): Promise<void> {
