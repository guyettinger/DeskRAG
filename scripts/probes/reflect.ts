...
const copy = cloneLibrary("deskrag-reflect-probe");
if (copy === null) process.exit(0);
const { root, data } = copy;

const { app, page } = await launchApp({ width: 1400, height: 1100, userDataDir: root });
/** The rows this probe measures, as `session_reflection` stores them. */
...
try {
  ...
  if (bail === null && target !== undefined) {
    ...
    const queued = must(
      await until(jobOf, { timeout: 30_000, every: 2000, label: "the job to appear" }),
      "the queued re-index job",
    );
      ...
      await until(
        async () => {
          ...
          process.stdout.write(
            `\r  ${j.state} ${j.done}/${j.total} stages · reflect=${st?.state ?? "-"}    `,
          );
          return j.state === "done" || j.state === "failed" ? j : null;
        },
        { timeout: 30 * 60_000, every: 2000, label: "the re-index to finish" },
      ),
      "the finished re-index job",
    );
    console.log("");
  ...
  try {
    ...
    console.log("could not read the clone:", err instanceof Error ? err.message : err);
  }
}
...
if (note !== undefined) {
  ...
  const leaked = renderedHabits.filter((md) => md.includes(firstLine));
