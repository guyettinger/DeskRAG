...
The Library player, the track rail, Flows, Skills, and the one global stylesheet. Nearly every rule here was found by measuring the running app, not by reading code.
...
- **The Skills screen is the only one that WRITES the user's own text**, and that is what makes its table AUTHORED rather than derived. Two panes on the Flows pattern — a banded list beside one skill's editor — and one selection. Every IPC call returns the whole `SkillsDTO`, the `settings.set` precedent, so the screen never re-fetches and cannot paint a half-applied edit.
  - **The bands are ordered by what needs answering**: Needs attention (re-bound, split, orphaned) leads, then Mine, then Proposed, then Archived. A skill whose evidence moved is the only thing on this screen that can be silently wrong, and a band sorting below the ones that are fine would hide exactly that. Within a band: pinned first, then `updatedAt` desc, so the thing just worked on is where it is looked for.
  ...
  - **The markdown is rendered in MAIN and copied verbatim.** `Copy SKILL.md` puts `skill.markdown` on the clipboard — the same string `get_skill` returns — so the renderer never re-renders the document. Two renderers of one file is the `ax-dump`/`ax-exec` drift hazard, and `npm run probe:skills` diffs the two byte for byte because nothing in the suite can.
  ...
  - **Four defects here were found by driving the app and reading a rendered file, not by the suite.** The template body claimed "no summary model was configured" on a machine that had one (`acceptSkill` writes a template deliberately, so keeping is instant — asserting a cause it cannot observe is the fabrication the file exists to avoid); every single-day skill read "between 2026-08-17 and 2026-08-17" because the span compared MILLISECONDS and two steps of one recording are minutes apart, so the equal case was unreachable; `describeTarget` returns an em dash for a chord or a wait, printing "`press cmd+v` — —"; and a `type` target already names its slot, so appending it said it twice. None of the last three was reachable from a fixture that did not happen to contain the shape.
...
- **The Flows screen is a READER, and that is its hard rule.** `main/graph-view.ts` is the pure `Graph` → DTO projection and is tested in the ROOT suite (`tsconfig.json` and `vitest.config.ts` both map `@shared/types` and `deskrag` for it — exact file paths, because the app resolves modules as a bundler and may omit extensions while the root is NodeNext and may not). `DeskRagService.flows()` is the only I/O, and there is exactly one IPC channel (`flows:graph`) with no subscriptions.
  ...
  - **A ROUTE IS KEYED BY THE STATES IT PASSES THROUGH, NAMED — and the two stricter keys were both measured and both failed.** On a real graph of 9 recordings / 17 nodes / 44 edges: the ordered **edge-id** sequence gave 9 routes all ×1 (five recordings of one Calculator task differed only in how many buttons were pressed — 4/5/6/8 steps), and the ordered **node-id** sequence also gave 9 routes all ×1, for a deeper reason: **equivalent states across recordings do not merge into one node**, because identity is task-derived and `matchNode` correctly declines when the outgoing edge targets a different element. Node identity cannot express "the same place" across recordings at all. The deduped `labelNode` sequence gave **5 routes, one of them ×5** — the right granularity because a label is app plus its distinguishing hint, which is the level at which a person says "I do this a lot". A predicate-less node is dropped from the key (vacuously true of every desktop, so it names no place); leaving it in put a meaningless `n0 — no state` at the head of all nine routes.
    - **DROPPING IT FROM THE KEY WAS NEVER ENOUGH — it stayed on the canvas and in every skill.** The route *label* was clean while the card `n0 — no state` still headed the graph and the numbered procedure in a kept SKILL.md still opened `1. n0 — no state → Electron — localhost — (no actions recorded on this edge)`. It is gone at the source now (`liftTrace`'s `startTMono`), which also removed the fake universal root it was acting as — see trace-and-replay.md. **The screen after: 28 states/61 actions → 22/49, zero `no state` cards, zero `no keyframe` cards, and a rank ladder of 3/3/7/4/3/1/1 where 15 nodes had been flat in one row.**
