...
- **The Indexing screen draws the pipeline, and its geometry says two things at once.** `IndexingScreen.tsx` is the queue plus one selected job's ladder; `StageGraph.tsx` draws it; `index-graph-view.ts` is the pure geometry (`.ts`, root-tested, the `graph-layout.ts` pairing). Main decides what a row MEANS (`index-graph.ts`), the renderer decides where it goes — the same split as `session-tracks.ts` versus `track-view.ts`.
  - **ROW IS EXECUTION ORDER; COLUMN IS DEPENDENCY DEPTH.** `runStages` is a strictly sequential `for` loop, so a free-branching DAG layout would assert a concurrency the app does not have — Captions, Transcribing and Frame patches all depend only on Segmenting and would sit side by side as if they ran together. Reading top to bottom therefore IS the run order, which also makes every wire point at a strictly smaller row. **Deriving the row from `needs` is wrong and silently so: `trace` has depth 2 and runs LAST**, because a graph accretes across recordings. The plan table already says this — "the array IS the order; `needs` is CHECKED, never obeyed."
  - **EVERY WIRE GETS ITS OWN CHANNEL, and the first version did not.** Bowing each wire into an 18px gutter by an amount tied to its span drew a vertical stripe of overlapping dashes that reads as static: **measured in the running app, 16 of 21 wires shared the same few pixels**, and the "distant" dash style meant to distinguish them had become the rule rather than the exception. Every wire was individually correct, every DOM assertion passed, and only the screenshot showed anything was wrong. `routeWires` now assigns channels the commit-graph way — shortest span first, and two wires may share a channel only when their row ranges do not even TOUCH, because contiguous vertical segments at one x read as a single wire. The gutter is SIZED from the channel count, so a thirteenth stage cannot push one off the canvas.
