...
**Architecture:** `src/replay/` is pure TypeScript that spawns a subprocess, so it is barrel-exported (the repo's line is *native module, not subprocess*). All actuation goes through an injected `Actuator` interface — `sidecar.ts` implements it over a new long-lived Swift binary, tests implement it as `FakeActuator`, and `execute.ts` never touches a process. Anchor resolution walks a five-rung ladder — `identifier` first, `point` last, and the middle ordered by how much each rung is trusted for that anchor — recording which rung won and how confident it is.

> **Post-execution note.** This plan was executed on 2026-07-31 and the code diverged from it in three places, each recorded here because the embedded code blocks below still show the original:
>
> 1. **The ladder order is computed, not fixed.** The plan shipped `identifier → path → label`; recording in Chrome and then System Settings falsified that and its replacement in turn. `path` is no longer in `LAYER_CEILING` — `pathCeiling(depth)` decays its trust from 0.95, crossing below a label's 0.8 just past depth 5, and `resolve.ts` sorts the available rungs by trust. AppKit paths average depth 4 and web paths 11.4, which is exactly why no fixed order serves both. See the spec.
> 2. **Task 7's Swift `locate` uses one traversal, not two.** The plan walked the tree twice and indexed across both arrays; a divergence in ordering would have silently acted on the wrong element.
> 3. **Task 5's first `buildPlan` test expected `identifier`**, but its fixture anchor has no identifier. It resolves at `label`.
