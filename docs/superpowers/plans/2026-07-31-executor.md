...
>
> 1. **The ladder order changed** from `identifier → path → label` to `identifier → label → path`, and `LAYER_CEILING` with it (`label` 0.8, `path` 0.6). Recording a second task in Chrome showed AXIdentifier on 9% of AX anchors against 50–80% in TextEdit, and web-content paths running 11–17 levels deep against 1–4 for native controls. Ranking by availability was abandoned for ranking by reliability — see the spec.
